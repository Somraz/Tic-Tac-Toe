#function to implement magic square
def magic():
...
a=magic()
#to check if square if occupied
#if value at index is 1, it is occupied
b=[]
#array of computer moves
comp=[]
#array of human moves
human=[]
for i in range(0,9):
    #all sqaures initially unoccupied
    b.append(0)
def pos_win(arr):
    flagh=0
...
def move_4():
    flag = 0
    #check for possible win of human
    n=15-(human[0]+human[1])
    if n>0 and n<=9 and b[n-1]==0:
        #blocks human
        b[n-1]=1
        comp.append(n)
    #else finds suitable non corner square
    else:
        done=0
...
def move_5():
    flag = 0
    #computer checks for its possible win
    n = 15 - (comp[0] + comp[1])
...
def move_7():
    ...
    if n1>0 and n1<=9 and b[n1 - 1] == 0:
        ...
        #the computer plays at any square unoccupied
        for i in range(0, len(b)):
            if b[i] == 0:
                b[i] = 1
                comp.append(i + 1)
...
def move_8():
    ...
    if n1>0 and n1<=9 and b[n1-1]==0:
        ...
        #plays the move on any empty square
        for i in range(0, len(b)):
            if b[i] == 0:
                done=1
                b[i] = 1
    ...
    if n1>0 and n1<=9 and b[n1 - 1] == 0:
        ...
        for i in range(0, len(b)):
            if b[i] == 0:
                ...
                break
    return flag
#function to print the board with all the moves
...
print(" ------   -   -------            ------    --------   -------           ------    --------   --------        \n"
      ...
      "   | |   | | |     \   \_____\     | |    |  | - | | |      \  \_____\    | |    |   \_/  | |      /             \n"
      "   | |   | | \      -              | |    |  |   | | \       -            | |    |        | |     /_              \n"
      "   |_|   |_|  \_____/              |_|     \_/    \|  \______/            |_|     \______/   \______\              \n")
...
t=int(input())
#variable to check if computer wins
f=0
board=[]
for i in range(0,9):
    board.append(str(arr[i]))
#let the first player always start with "X"
ansh=""
ansc=""
...
for j in range(0,9):
    if (t==0 and j%2==0) or (t==1 and j%2!=0):
        ...
        #All computer moves according to algorithm
        ...
        for q in range(0,9):
            if arr[q]==s:
                #computer move is marked on board
                board[q]=ansc
                break
    #print board position after each move
    print_board(board)
